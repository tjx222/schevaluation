...
function initIndicator(data) {
	var oneIndicator = data.childIndicators;
		for ( var i in oneIndicator) {
			showIndicatorResultBuffer(oneIndicator[i].resultList);
...
function showSecondIndicator(indicator){
	...
	function showIndicatorResult(indicator,leftheight) {
		...
		_tds += 
			...
			+' data-indicatorScore='
			+indicator.scoreTotal
			+'><select onchange="evlOperatorWeight(this);" class="select_over">' 
  ...
  //保存问卷填报弹窗
    ...
    function not_submit_dialog_close(){
  	  $("#not_submit_dialog").dialog("close");
    }
