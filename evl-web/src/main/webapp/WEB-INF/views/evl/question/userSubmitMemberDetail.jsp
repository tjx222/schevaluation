...
<div class="see_teaching_evaluation"  style="padding-bottom:0;min-height:668px;">
	...
	<form id="userForm" action="${ctx}jy/evl/question/getQuestionMemberDetail"  method="post">
	...
	<div class="see_teaching_evaluation_option">
		...
		<div class="a1 a1_width">
			...
			<select class="chosen-select-deselect classId" name="classId" style="width: 80px; height: 25px;" onchange="formSubmit();">
				...
				<option value="${class_.key}" ${class_.key==m.classId?'selected':''}>${class_.value}</option>
				</c:forEach>
			</select>
		</div>  
		<div class="a1 a1_width">
			...
			<select class="chosen-select-deselect gradeId" name="gradeId" style="width: 80px; height: 25px;" onchange="formSubmit('nj')">
				...
				<option value="${grade.key}" ${grade.key==m.gradeId?'selected':''}>${grade.value}</option>
				</c:forEach>
			</select>
		</div>  
	...
	<div class="see_teaching_evaluation_table">
		<table>
			...
			<c:forEach items="${pageList.datalist}" var="user">
				...
				<td>${user.status==member_tijiao?"已评":"未评"}</td>
			</tr>
			</c:forEach>
		</table>
	</div>
	<div class="pages">
		<form id="pageForm" name="pageForm" method="post">
			<ui:page url="./jy/evl/question/getQuestionMemberDetail" data="${pageList}" views="12"/>
...
function downloadQuestionMemberDetail(){
	...
	window.location.href="${ctx}/jy/evl/question/downloadQuestionMemberDetail?questionId="+questionId+"&gradeId="+gradeId+"&classId="+classId+"&status="+status;
}
