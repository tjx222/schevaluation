...
public class DateUtils extends org.apache.commons.lang3.time.DateUtils {
  ...
  public static long getWeeksBetween(Date start, Date end) {
    ...
    calendar_start
        .set(Calendar.DAY_OF_WEEK, calendar_start.getFirstDayOfWeek());
    log.debug("开始时间==="
        + formatDate(calendar_start.getTime(), "yyyy-MM-dd HH:mm:ss"));

    calendar_end.set(Calendar.DAY_OF_WEEK, calendar_end.getFirstDayOfWeek());
    log.debug("结束时间==="
        + formatDate(calendar_end.getTime(), "yyyy-MM-dd HH:mm:ss"));

    long diff = calendar_end.getTimeInMillis()
  ...
  public static boolean isOverCurrentTime(Date time) {
    String date1 = getDate("yyyy-MM-dd HH:mm");
    Date parseDate = parseDate(date1);
    return time == null ? false : parseDate.getTime() >= time.getTime();
  }
}
