package com.tmser.schevaluation.common.orm;
...
public abstract class OrmHelper {
   ...
   *           sql异常
   */
  public static <T> T mapperBean(ResultSet rs, int rowNum, Class<T> classType,
      boolean primitivesDefaultedForNullValue) throws SQLException {
        ...
        try {
          ...
          if (value == null && primitivesDefaultedForNullValue) {
            log.debug("Intercepted TypeMismatchException for row {}"
                ...
                rowNum, column, value, column.getName(), column.getAttrType(),
                t);
          } else {
            throw e;
          }
  ...
  public static Table parseTable(ScannedGenericBeanDefinition beanDefinition) {
    ...
    try {
      ...
      try {
        c = beanDefinition.resolveBeanClass(OrmHelper.class.getClassLoader());
      } catch (ClassNotFoundException ec) {
        log.error("{} doesn't found!", c == null ? "" : c.getName());
        return null;
      }
    }
    ...
    if (metadata.hasAnnotation("javax.persistence.Table")) {
      ...
      t.setCatalog((String) AnnotationUtils
          .getValue(tableAnnotation, "catalog"));
      t.setTableName((String) AnnotationUtils.getValue(tableAnnotation, "name"));
      t.setSchema((String) AnnotationUtils.getValue(tableAnnotation, "schema"));
    ...
    if (transientAnnotation == null) {
      if (annotation != null) {
        ...
        clm.setColumn(StringUtils.defaultIfBlank(
            (String) AnnotationUtils.getValue(annotation, "name"), f.getName())
            .toUpperCase());
        clm.setNullable((Boolean) AnnotationUtils.getValue(annotation,
            "nullable"));
        clm.setLength((Integer) AnnotationUtils.getValue(annotation, "length"));
        clm.setUnique((Boolean) AnnotationUtils.getValue(annotation, "unique"));
        clm.setColumnDefinition((String) AnnotationUtils.getValue(annotation,
            "columnDefinition"));
      ...
      if (f.isAnnotationPresent(javax.persistence.Id.class)) {
        ...
        if (f.isAnnotationPresent(javax.persistence.GeneratedValue.class)) {
          ...
          if ((GenerationType) AnnotationUtils.getValue(gn, "strategy") != null) {
            ...
            clm.setGenerationType((GenerationType) AnnotationUtils.getValue(gn,
                "strategy"));
          }
        }
      }
